...
The artifact targets Java 17/21 runtimes via `maven.compiler.release=17` (safe to cross-compile from any newer JDK), so the build must never cap the JDK. The one thing that ever did was mockito's bundled byte-buddy rejecting class files newer than it knows; mockito was dropped for hand-rolled fakes (`FakeRepositorySystem`, `NoopEnforcerLogger`, a real `MavenSession`), so no bytecode-generating test tool caps the JDK anymore — keep it that way. When a new JDK GA lands, just bump the CI matrix's newest entry.
